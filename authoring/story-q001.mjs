...
s.story.revision=2;s.resetScripts=true;
...
s.brief='篝火の迷宮の巡灯路から、当直の老灯番が戻りません。実地教育に同行した新人の帰還も確認してください。';
...
const mapAt=(x,y)=>({op:'map.teleport',map:'kagaribi_f1',x,y,facing:'east'});
s.sceneCommands={entry:[mapAt(2,1)],dark:[mapAt(9,1)],empty:[mapAt(9,1)],old:[mapAt(13,3)],outage:[mapAt(9,1),portable(0)],rescue:[mapAt(9,1),portable(25)],post:[{op:'town.return'}]};
s.node('entry','entry',['rookie'],'入口の篝火の下で、新人が松明を両手で握っていた。「壁の松明か、くらがり除けの火を持っていれば、あれは寄れません。でも、巡灯路の火が消えて……あの人が、自分の油を僕にくれたんです」。奥で石をこする音がする。新人は一歩を出そうとし、足を引いた。「戻らなきゃいけないのに、怖くて」。',[
 O('talk','火を確かめ、老人を探しに行く。新人には入口の灯を守ってもらう','dark',[see('rule','rookie'),see('gift','rookie'),move('party','entry','dark')],{commands:[portable(25),mapAt(9,1)]})
]);
...
s.node('empty','dark',[],'火を芯へ寄せると、一瞬だけ先端が赤くなり、すぐ消えた。油受けは底まで乾いている。芯の向きや点火の仕方ではなく、壁松明の油そのものが尽きていた。二つ目も同じだ。手元の松明を掲げ、杖の音へ進む。',[
 O('follow','携行松明を頼りに、杖の音がする支道へ進む','old',[move('party','dark','branch'),see('position','elder'),set('partyTorch','low')],{commands:[portable(8),mapAt(13,3)]})
]);
s.node('old','branch',['elder'],'老人は小さく燃える壁松明の真下にいた。油受けの底が見える。「あの子は入口まで行けたか」。頷くと、空の油瓶を伏せた。「あれは新人に持たせた。わしは、ここの火が消えるまでに誰か来てくれればと思ってな」。老人の足は腫れ、立つには肩が要る。携行松明にも油は残り少ない。',[
 O('support','老人に肩を貸し、最後の壁灯があるうちに入口へ戻る','outage',[move('party elder','branch','dark'),set('partyTorch','extinguished'),set('darkness','attacking')],{commands:[wall('q001_last_lamp','extinguished'),mapAt(9,1),portable(0)]})
]);
s.node('outage','dark',['elder'],'曲がり角で、携行松明の炎が縮んだ。油はもう出ない。最後の赤い芯が暗くなると、往路で火の外にいたものが、一息で距離を詰めてきた。老人の腕が肩に食い込む。後ろの壁灯も消えている。',[
 O('call','老人を壁際に支え、入口へ向かって声を上げる','rescue',[{op:'consume',key:'newOil',amount:1,sink:'oilUsed'},move('rookie','entry','dark'),set('steppedForward'),set('darkness','repelled'),see('returningFire','rookie')],{commands:[portable(25)]})
]);
s.node('rescue','dark',['elder','rookie'],'揺れる火が、入口の方から来た。新人の膝は震え、松明を差し出す手も定まらない。「怖いです。今も。でも、二人とも、ここにいるから」。老人にもらった油で灯した火を前へ出す。くらがりが身をよじり、明かりの外へ退いた。老人は空の瓶を見て、新人の火を見た。「返しに来たか」。',[
 O('home','新人の火を頼りに、三人で詰所へ帰る','post',[move('party elder rookie','dark','entry','post'),set('reported'),set('routeClosed')],{commands:[{op:'town.return'}]})
]);
