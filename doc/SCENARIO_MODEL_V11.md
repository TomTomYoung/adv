...
値：`game.version: 1.9.0` / 意味：配布作品の内容版。保存内容の一致検査に使う
...
値：`story.revision` / 意味：同一形式内の状態構成の改訂。省略時は1。q001は火と恐怖、q008は外底追加により2
...
1.9.0は旧内容版のセーブを移行しません。不正または旧版の記録は最初から開始します。同版の記録では、型・実体の所在・情報参照・同席・結末成立を検証後に置き換えます。未知の状態名、数量超過、保持関係の循環、未成立の救出記録はエラーです。

q004のwindow別名とq008の同内容版内の改訂検証は残っています。旧作品版への移行保証ではありません。q001は改訂2だけを生成し、以前の新人同行・先行引継ぎ・油分配の経路を廃止しました。
