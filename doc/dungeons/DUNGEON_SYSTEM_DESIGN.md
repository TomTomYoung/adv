...
通常移動・ワープ・階段・退場と、地形が足元を塞ぐ場合の扱いも検証します。部品・保存構造を変える場合は内容版を更新します。現行版は旧内容版の移行を用意せず、読込エラー時に新規開始します。

## シナリオと表示

現地調査は `config/quests/qXXX.events.json` に定義し、クエストJSONへ集約します。素材は `config/dungeon-art.json` に定義します。実状態を読む調査、手帳への記録、画像の割当を実装しています。操作中の本筋のフラグをビューが直接変更する設計にはしません。
...
現行JSONで表現できない規則を、未登録の `use` や任意JavaScriptの文字列として追加してはいけません。条件付きイベントは `fieldEvents` に購読通知・条件式・再発・処理を定義し、[イベント仕様](../scenarios/EVENT_SYSTEM.md)と登録済みJSON DSLの範囲で編集できます。既存の条件・処理ブロックを超える新しい規則にはCore側の実装が必要です。
