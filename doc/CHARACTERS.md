...
更新: 2026-09-13。q001〜q010 の登場人物は安定した ID で定義し、会話場面に AIPaint 製の画像を表示する。同じリネを別人として増やさず、関所番と水門番は分ける。名無しの役割に本名を捏造せず、集団は集団実体として記載する。
...
| ID | 名前・役割 | 登場 | 動機 |
|---|---|---|---|
| rine | リネ／灯番組合の連絡係 | q001, q010 | 帰還者を名前で数え、取り残しを防ぐ |
...
| recipient | 下流の受取人／避難民側の食料受取担当 | q008 | 食料を受け取り、待つ人々へ配る |
| inspector | 輸送検査官／食料輸送の検査担当 | q008 | 今回の荷と届け先を確かめる |
| eda | エダ／地下測量士 | q009 | 鼠の異変と構造物の危険を切り分ける |
| cleaners | 清掃係／石灰消毒の担当者 | q009 | 担当区画を清潔に保つ |
| storekeeper | 穀倉番／穀物庫の管理者 | q009 | 鼠から穀物を守る |
| clerks | 共同倉庫の受入れ係／数量と配分の記録担当 | q009 | 搬入数と出庫先を公開して管理する |
| gatekeeper | 水門番／老朽水門の操作担当 | q010 | 命令と取り残した住民の間で責任を決める |
| nearpeople | 手前の住民三人／避難通路で待つ住民 | q010 | 三人で地上へ出る |
| deeppeople | 奥の住民二人／作業区画で待つ住民 | q010 | 渡りを越えて地上へ出る |
| pumpcrew | 排水隊／閉門を延期する交代班 | q010 | 現場を引き受け、避難の時間を稼ぐ |

### リネ (rine)

![リネ](../assets/images/characters/rine.png)

q001とq010は同一人物。依頼受付・救助名簿・報告を担当する。

[AIPaint 編集原稿](../assets/source/characters/rine.paint.json) ／ [描画コマンド](../assets/source/characters/rine.commands.json)

### 老灯番 (elder)

![老灯番](../assets/images/characters/elder.png)

本名は未設定。支道で待つ老人は、先行救助後に詰所の寝台に残る。

[AIPaint 編集原稿](../assets/source/characters/elder.paint.json) ／ [描画コマンド](../assets/source/characters/elder.commands.json)

### 新人灯番 (rookie)

![新人灯番](../assets/images/characters/rookie.png)

本名は未設定。老人から受け取った油を保持する。

[AIPaint 編集原稿](../assets/source/characters/rookie.paint.json) ／ [描画コマンド](../assets/source/characters/rookie.commands.json)

### 灯番救助隊 (rescuers)

![灯番救助隊](../assets/images/characters/rescuers.png)

集団実体。人数や隊員名は固定せず、代表像を表示する。

[AIPaint 編集原稿](../assets/source/characters/rescuers.paint.json) ／ [描画コマンド](../assets/source/characters/rescuers.commands.json)
...
![標本係](../assets/images/characters/curator.png)

番号照合と回収を手伝う。

[AIPaint 編集原稿](../assets/source/characters/curator.paint.json) ／ [描画コマンド](../assets/source/characters/curator.commands.json)
...
![ソラ](../assets/images/characters/sora.png)

鐘の機構を見張り、運用試験を担当する。

[AIPaint 編集原稿](../assets/source/characters/sora.paint.json) ／ [描画コマンド](../assets/source/characters/sora.commands.json)

### 通行人たち (passers)

![通行人たち](../assets/images/characters/passers.png)

集団実体。鐘を止める前に高所へ誘導する。

[AIPaint 編集原稿](../assets/source/characters/passers.paint.json) ／ [描画コマンド](../assets/source/characters/passers.commands.json)
...
![水位見張り当番](../assets/images/characters/waterwatch.png)

人手で鳴らす警報に必要な交代制の集団。

[AIPaint 編集原稿](../assets/source/characters/waterwatch.paint.json) ／ [描画コマンド](../assets/source/characters/waterwatch.commands.json)

### イナ (ina)

![イナ](../assets/images/characters/ina.png)

写しを持つ妹。姉の原本と同じ資格番号を使った。

[AIPaint 編集原稿](../assets/source/characters/ina.paint.json) ／ [描画コマンド](../assets/source/characters/ina.commands.json)

### イナの姉 (sister)

![イナの姉](../assets/images/characters/sister.png)

原本の資格者。本名は未設定。

[AIPaint 編集原稿](../assets/source/characters/sister.paint.json) ／ [描画コマンド](../assets/source/characters/sister.commands.json)

### 関所番 (passkeeper)

![関所番](../assets/images/characters/passkeeper.png)

水門番とは別人。死んだ兄は歴史上の人物で、生存NPCではない。

[AIPaint 編集原稿](../assets/source/characters/passkeeper.paint.json) ／ [描画コマンド](../assets/source/characters/passkeeper.commands.json)

### 名義審査官 (reviewer)

![名義審査官](../assets/images/characters/reviewer.png)

現場へ赴き申告を確認し、姉妹の仮証を発行する。

[AIPaint 編集原稿](../assets/source/characters/reviewer.paint.json) ／ [描画コマンド](../assets/source/characters/reviewer.commands.json)
...
![工事の職人たち](../assets/images/characters/workers.png)
...
![ミレ](../assets/images/characters/mire.png)

夫は故人。弟との面会や伝声口の封鎖を本人が選ぶ。

[AIPaint 編集原稿](../assets/source/characters/mire.paint.json) ／ [描画コマンド](../assets/source/characters/mire.commands.json)
...
![ヨル](../assets/images/characters/yoru.png)

遺体を納める内室と別の外底に食料を隠した。

[AIPaint 編集原稿](../assets/source/characters/yoru.paint.json) ／ [描画コマンド](../assets/source/characters/yoru.commands.json)

### 棺の遺族 (family)

![棺の遺族](../assets/images/characters/family.png)

外底の検査と葬儀の延期には了承を得る。

[AIPaint 編集原稿](../assets/source/characters/family.paint.json) ／ [描画コマンド](../assets/source/characters/family.commands.json)

### 棺の担ぎ手 (bearers)

![棺の担ぎ手](../assets/images/characters/bearers.png)

棺の移動経路を家族や食料と分けて管理する。

[AIPaint 編集原稿](../assets/source/characters/bearers.paint.json) ／ [描画コマンド](../assets/source/characters/bearers.commands.json)
...
![輸送検査官](../assets/images/characters/inspector.png)

臨時許可を審査する。恒久的な制度変更はここでは決定しない。

[AIPaint 編集原稿](../assets/source/characters/inspector.paint.json) ／ [描画コマンド](../assets/source/characters/inspector.commands.json)
...
![清掃係](../assets/images/characters/cleaners.png)

合意した区画表は保存する。散布済みの薬は元に戻らない。

[AIPaint 編集原稿](../assets/source/characters/cleaners.paint.json) ／ [描画コマンド](../assets/source/characters/cleaners.commands.json)

### 穀倉番 (storekeeper)

![穀倉番](../assets/images/characters/storekeeper.png)

内扉を閉め、穀物への侵入を防いでいる。

[AIPaint 編集原稿](../assets/source/characters/storekeeper.paint.json) ／ [描画コマンド](../assets/source/characters/storekeeper.commands.json)

### 共同倉庫の受入れ係 (clerks)

![共同倉庫の受入れ係](../assets/images/characters/clerks.png)

穀物を受け取った事実を記録する集団。

[AIPaint 編集原稿](../assets/source/characters/clerks.paint.json) ／ [描画コマンド](../assets/source/characters/clerks.commands.json)
...
![手前の住民三人](../assets/images/characters/nearpeople.png)

人数三人を固定。地上到着前に救出済みにしない。

[AIPaint 編集原稿](../assets/source/characters/nearpeople.paint.json) ／ [描画コマンド](../assets/source/characters/nearpeople.commands.json)

### 奥の住民二人 (deeppeople)

![奥の住民二人](../assets/images/characters/deeppeople.png)

人数二人を固定。渡りには救助縄が必要。

[AIPaint 編集原稿](../assets/source/characters/deeppeople.paint.json) ／ [描画コマンド](../assets/source/characters/deeppeople.commands.json)

### 排水隊 (pumpcrew)

![排水隊](../assets/images/characters/pumpcrew.png)

連絡だけで引継ぎ済みとせず、操作室への到着を確認する。

[AIPaint 編集原稿](../assets/source/characters/pumpcrew.paint.json) ／ [描画コマンド](../assets/source/characters/pumpcrew.commands.json)

## 歴史上・物語内で言及される人物

| 人物 | 関係 | 扱い |
|---|---|---|
| ミレの夫 | q007、弟の兄 | 故人。声の主ではない。生存 NPC の所在を持たせない。 |
| 関所番の兄 | q004 | 故人。関所番が使う資格の名義人。 |
| 棺の故人 | q008 | 遺体を物品 body として棺の内室に保持。生存 NPC として表示しない。 |

## 既存の探索隊員

| ID | 名前 | 役割 |
|---|---|---|
| ada | アダ | 32歳。元水門警備隊。新人にも必ず帰り道を教える剣士。 |
| nio | ニオ | 24歳。地下の配達人出身。道を覚えるため毎晩靴底を描く斥候。 |
| sera | セラ | 40歳。町の施療所から来た祈祷師。帰った後の食事まで気に掛ける。 |
| il | イル | 29歳。書庫を追われた魔術師。暗所で読める灯火の術を磨いた。 |
| berg | ベルグ | 54歳。引退した守衛。遅い足取りで誰より先に出口を確かめる。 |
| luka | ルカ | 27歳。救助隊の槍使い。届く距離を測り、無理な一歩を踏まない。 |
| toma | トーマ | 36歳。坑道の薬師。持ち運べる薬と集団の応急手当を研究する。 |
| mica | ミカ | 24歳。機関廟で学んだ符術師。凍結と雷を使い分けて守りを崩す。 |
| dora | ドーラ | 46歳。石工の盾兵。崩れた天井の下で人を庇った経験を持つ。 |
| ren | レン | 31歳。旅の楽師。眠気を払う節と呼吸を整える声掛けが得意。 |
