...
type：quest.travel / フィールド：id: 受注中の依頼ID。実行時の進行から次イベントの場所を再取得し、配置先ダンジョンの入口へ入る。クライアントからの固定ダンジョンIDは使わない。次が町のイベントなら出発しない。町の出発地点・会話中・戦闘中・探索中の制限を守る。
...
掲示板は受注・メインクエスト指定・入口ボタンの構成とし、現地調査の重複入口ボタンと右欄の入口ボタンを除去。右欄はメインクエストの次の場所を案内する。手帳でもメイン指定と入口操作ができ、広場にはメインクエストの入口ボタンを置く。組合の掲示板からは「灯帰り・篝火広場へ戻る」で戻る。迷宮に入る操作は広場から有効になる。
