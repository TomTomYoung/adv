...
2. 町・探索・会話・選択肢・戦闘・手帳の表示例を切り替えます。
...
プレビューは `data/view-fixtures.json` の6つの固定スナップショットを読みます。ゲーム本体や個人のセーブを実行しません。ボタンを押した際は送出する操作を表示します。表示例を最新データから再作成する開発用ツールは `node tools/build-fixtures.mjs` です。生成ツールだけがエンジンを利用し、表示ページからは読み込みません。
...
| party[] | id、表示名、職業、HP/MPと上限、状態異常、能力値、装備名 |
...
| battle | 手番、行動者、敵とHP/画像、使用可能な技能と道具、逃走可否、ログ |
...
保存メニュー・音声・全体キー入力は `src/main.js` のブラウザアダプターです。別のネイティブUIやCLIを作る場合はGameEngineとprojectGameを再利用し、このアダプターとGameViewを置き換えます。
