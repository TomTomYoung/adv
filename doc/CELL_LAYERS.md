...
通常石床、土床、木床、濡れた石床、浅水路、深水路、完全水没通路、空気溜まり、毒沼、腐食床、滑る氷床、崩れやすい床、呪いの流路、術封じ床、安全地帯、石壁、岩盤、土壁、塩壁、茨壁の生育床、根橋予定地、穴・縦坑、足場付き空間、足場なし空間の24種類です。以前の「石壁・岩盤」を別候補に分けています。個々の名称・説明・設定値は[カタログ](CELL_CATALOG.md)の原稿から生成した一覧を参照してください。
...
配置は `legend` が一文字とプリセットIDを結び、`rows` がその文字を並べます。`F` と `W` は配置原稿内の凡例であり、エンジンに固定されたセル種ではありません。現在は名前と用途説明を持つ24種類を登録しています。既存の仕掛けの361セルを対応する種類へ分類し、全33マップの初期通行値・接続・配置物は維持しました。毒沼や氷床などの新しい危険は既存シナリオ経路へ自動追加していません。
