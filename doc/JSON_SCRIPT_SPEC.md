> 実装状況（2026-09-09）: この文書は拡張先を含む設計原本です。現行1.0.0の実行可能な構文は [SCRIPT_REFERENCE.md](SCRIPT_REFERENCE.md)、実装範囲は [SPEC.md](SPEC.md) を参照してください。未対応の命令をこの原本からそのまま投入しないでください。
