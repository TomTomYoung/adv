...
更新日: 2026-09-14。対象は作品版1.8.0です。現在のマップに存在する種類と、操作によって発生する状態を整理しています。2Dマップも継続して保守する対象です。

地形データの文字種は `.`（空）と `#`（密）の二つです。水・足場・扉・植物・術の区域などは、地形に重ねる状態や配置物として定義されています。本書の分類名を、新しい `tile` の値としてJSONへ書き込むことはできません。

各方向の名称と判定順は[セルと境界の仕様](MAP_CELLS_AND_BOUNDARIES.md)、水の容量計算は[立体地形と水](VOXEL_TERRAIN_AND_WATER.md)、固有環境の概要は[ダンジョン一覧](DUNGEON_CATALOG.md)を参照してください。末尾の配置索引には、件数・座標・実際のID・編集元へのリンクを配布データから生成します。
...
下が密セル、または下面の `support` がtrueである空間です。乾燥・足元・腰の水深なら、境界と扉が通行可能な限り同じzから歩いて入れます。梯子などの到着先にもなります。足場があっても完全水没していれば通常進入できません。
...
範囲外はセル種ではありません。人物の移動先にはできず、3Dで通水可能な開口がつながる場合は排水先として扱います。

## 水のあるセル

### 乾燥：waterDepth = 0

水による移動制限がない状態です。壁・扉・足場による制限は残ります。貯水立坑は初期水量が空で、給水操作から水深が変化します。

### 足元まで：waterDepth = 1

通常通行できる浅水です。3Dでは1単位、2Dの地下水道では内部水位1の浅水をこの表示へ変換します。水面は床上へ描画し、壁や不透明な障害物にはしません。浅水であることだけを理由とする追加ダメージや移動コストはありません。

### 腰まで：waterDepth = 2

3Dで水が2単位ある状態です。足場と境界の条件を満たせば通常通行できます。旧2D地下水道の内部値2は完全水没を意味するため、この腰までの状態と混同しません。

### 完全水没：waterDepth = 3

3Dでは3単位で満水となり、通常進入と経路中の通過を拒否します。梯子・縄の設置だけでは解除しません。排水や別の高さの足場が必要です。水没は視線を遮る石壁へ変換しません。足元が給水などで通行不能になった場合は、通行できた経路を使う退避、または帰還印による救助を行います。

### 周期水域：waterworks / tidal

灯守の地下水道の低い通路です。水位周期で乾燥・浅水・完全水没を繰り返し、水位の低下を待てます。対応する水門を閉じると止水・排水して通行可能になります。周期の内部値0/1/2を表示水深0/1/3へ変換します。地形による貯水容量は計算しません。

### 常時水路：waterworks / channel

同じ地下水道の横断水路です。対応バルブが開いている間は完全水没し、待機だけでは水が引きません。バルブを閉じて通路を開きます。水域とバルブの操作位置は別のセルです。
...
作品版1.8.0。以下の件数と配置例は npm run build:docs で更新します。配置定義を数えるため、条件不成立・旧セーブ互換用のオブジェクトも含みます。空・密の件数は元の地形、足場と水深は初期地形状態です。探索後の地形や同時に有効なイベント数ではありません。
...
map.objects は計523定義、8種、配置座標は523か所です。件数はイベント定義数で、別の種別が同じ座標にある場合があります。safe は通常のランダム遭遇判定の抑止であり、仕掛けやスクリプトによる戦闘まで無効にする値ではありません。

`chest`：20定義。進入時0／調べる20、blocking指定0、safe指定20、once指定20。配置例：`cache` region_1_f1 (1,4) ／ `cache` region_1_f2 (1,4)。

`clue`：203定義。進入時0／調べる203、blocking指定0、safe指定201、once指定0。配置例：`q001_clue_a` region_1_f1 (7,1) ／ `q001_clue_b` region_1_f1 (14,1)。

`decision`：202定義。進入時0／調べる202、blocking指定0、safe指定202、once指定0。配置例：`q001_decision` region_1_f1 (13,7) ／ `q002_decision` region_1_f1 (11,9)。
...
火台10か所。`entry` kagaribi_f1 (1,1) ／ `watch` kagaribi_f1 (6,1) ／ `calm` kagaribi_f1 (8,5) ／ `crossroads` kagaribi_f1 (9,7) ／ `landing` kagaribi_f2 (1,1) ／ `lure` kagaribi_f2 (7,3) ／ `refuge` kagaribi_f2 (11,7) ／ `deep_landing` kagaribi_f3 (1,1) ／ `last` kagaribi_f3 (7,3) ／ `origin` kagaribi_f3 (7,7)。
...
`tidal` 上層の低い通路 (upper_basin)：2セル。region_1_f1 (3,2) ／ region_1_f1 (3,3)。制御：upper_gate。

`tidal` 下層の低い通路 (lower_basin)：3セル。region_1_f2 (5,5) ／ region_1_f2 (6,5) ／ region_1_f2 (7,5)。制御：lower_gate。

`channel` 上層の横断水路 (upper_channel)：1セル。region_1_f1 (4,3)。制御：upper_valve。

`channel` 下層の横断水路 (lower_channel)：1セル。region_1_f2 (6,1)。制御：lower_valve。

周期：干潮 12刻／内部水位0 → 増水 6刻／内部水位1 → 満水 8刻／内部水位2 → 減水 6刻／内部水位1。制御点4か所：`upper_gate` region_1_f1 (2,1) ／ `upper_valve` region_1_f1 (2,3) ／ `lower_gate` region_1_f2 (2,1) ／ `lower_valve` region_1_f2 (5,2)。
...
個別のセル指定なし。ダンジョン内の戦闘開始ごとに装備の正の補正を1ずつ減らします。
...
装置 `hand_pump` 手押し給水ポンプ：pump、操作(3,2,0)、対象(4,2,0)。給水2単位。
