...
作品版1.9.0。以下の件数と配置例は npm run build:docs で更新します。配置定義を数えるため、条件不成立・過去経路のオブジェクトも含みます。空・密の件数は元の地形、足場と水深は初期地形状態です。探索後の地形や同時に有効なイベント数ではありません。
...
`clue`：204定義。進入時0／調べる204、blocking指定0、safe指定199、once指定0。配置例：`q002_clue_a` region_1_f1 (1,8) ／ `q002_clue_b` region_1_f1 (3,13)。
