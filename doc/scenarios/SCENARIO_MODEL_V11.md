...
作品版: 1.20.0。更新日: 2026-09-25。v1.1の状態モデルは1.4.0から導入しています。
...
値：`game.version: 1.20.0` / 意味：配布作品の内容版。保存内容の一致検査に使う
...
現行版は旧内容版のセーブを移行しません。不正または旧版の記録は最初から開始します。同版の記録では、型・実体の所在・情報参照・同席・結末成立を検証後に置き換えます。未知の状態名、数量超過、保持関係の循環、未成立の救出記録はエラーです。
