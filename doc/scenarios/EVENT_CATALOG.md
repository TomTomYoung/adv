...
作品版 1.20.0。配布データから生成する実装一覧。[q001の位置・全文](QUEST_Q001.md) ／ [イベント仕様](EVENT_SYSTEM.md) ／ [命令仕様](SCRIPT_REFERENCE.md) ／ [セル照明](../FIELD_LIGHTING.md)。
...
通常のくらがり襲撃は条件付き即戦闘から歩行遭遇へ変更した。通常点灯はくらがりを防ぎ、普通の魔物は出る。火の守りがない場所では成功7歩ごとに基本22％で抽選し、敵候補は現在くらがり100％。消火操作だけでは戦闘しない。q001の老人同行時の消灯は独立したイベント戦闘で、確率・周期を通さない。[火と遭遇](../KAGARIBI_DUNGEON.md)。
