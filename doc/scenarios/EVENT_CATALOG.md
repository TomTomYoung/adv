...
作品版 1.16.0。配布データから生成する実装一覧。[q001の位置・全文](QUEST_Q001.md) ／ [イベント仕様](EVENT_SYSTEM.md) ／ [命令仕様](SCRIPT_REFERENCE.md) ／ [セル照明](../FIELD_LIGHTING.md)。
...
本編のセルイベント定義は0件。今回の移行では新たな危険床を配置しない。環境変化の購読とfire_network.dangerによるくらがり開始の統合は後続作業。
