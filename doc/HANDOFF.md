...
帰路の暗区間でプレイヤー側の松明が消え、火を避けていた「暗がり」が襲来する。暗がりは序盤から火の外側で気配を見せるなど、火がある間は寄れず、完全な暗闇で襲う存在であることを事前に示す。唐突な襲撃にしない。
...
状態設計では「恐怖が消えた」ことを成立条件にしない。新人の恐怖は残ったままでもよく、「恐怖があるにもかかわらず助けに動いた」という行為を別状態・行動履歴として表現する。必要なら `entry` と `branch` の間に暗区間を物語上の場所として追加し、松明の点灯状態、壁灯の消灯、暗がりの出現・退去、新人の所在地、老人の所在地をシナリオJSONの状態とイベント条件で追跡できるようにする。
