...
確認日: 2026-09-19。作品版1.14.0。起点はPR #29反映済みのmaster `59abddd`。作業ブランチは `fix/dungeon-direction-controls`。ダンジョン探索の基準を方向キーによる直接移動・決定による調査へ修正し、選択画面との切り替え、キー設定の案内、UI文書を更新しました。Core・配布JSON・セーブ形式は変更していません。
...
1. PR #28の共通キー操作、PR #29のキー設定、今回の探索と選択の切り替えは実ブラウザ確認が残っています。PR #27の公開画面では「記録 → 画面配置」と両ビューへの切り替えを確認済みです。ローカルプレビューは `ERR_BLOCKED_BY_CLIENT` で拒否されました。PR #28の23件のキー試験と既存10件の画面試験はNodeのDOMアダプターを使い、実際のCSS・ネイティブフォーカスの検証には代えません。公開後、ダンジョンの直接移動と決定の調査、選択肢が出た際の移動停止と選択終了後の探索復帰、選択肢の先頭、敵・味方対象、装備と転職、記録枠と設定、新規開始確認から戻る経路に加え、キー変更の入力待ち・取消・適用・初期値復帰・再読み込み後の保持を確認してください。320〜1440px・低い横画面・本文24pxで候補一覧とフォーカス枠の収まりも確認してください。
