...
### クエスト固有の現地イベントをクエストJSONへ集約する

現行の `dungeons[].fieldScenes` と `authoring/dungeon-scenes.json` に置かれている `quest`、`points`、`condition`、`script`、`note` などのクエスト固有定義は、すべて対応する `qXXX` の原稿・クエストJSONへ移す。クエストに属する会話、観察、条件、対象地点、関連ダンジョンへの参照はクエストJSONを正本とし、ダンジョン側からクエストを逆参照する構造を廃止する。

ダンジョン定義側には `maps`、`systems`、`art`、入口など、そのダンジョン自体の構造と固有システムだけを残す。クエストJSON側から必要なダンジョン・マップ・地点を参照する方向に統一する。`fieldScenes` という独立概念を残す必要があるかもこの移行時に再検討し、単なるクエスト内イベントとして表現できるものはクエストの進行・状態・scriptsへ統合する。

移行時は現行の調査済み状態、script ID、旧セーブ互換を壊さないこと。`projectFieldLinks`、`projectFieldNotes`、`dungeonScenePlan`、検証処理、UIの「現地調査へ向かう」もクエスト側の定義を読むよう変更し、ダンジョン側にクエスト固有データが残っていないことを検証する。全13件の現行 `fieldScenes` を移行対象として棚卸しする。
