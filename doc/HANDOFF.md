...
確認日: 2026-09-19。作品版1.14.0。起点はPR #27反映済みのmaster `5eae7c1`。作業ブランチは `feat/keyboard-controls`。会話・戦闘・装備・記録の共通キー操作と、広場・探索へ戻るキャンセルを追加しました。Coreと配布JSONは変更せず、Applicationに取消先と道具対象の投影を追加しています。
...
[KEYBOARD_CONTROLS.md](KEYBOARD_CONTROLS.md)が操作仕様です。`keyboard.js` はEnter・矢印・Escapeを共通処理し、`focus.js` が有効なボタンへの移動、選択欄の候補一覧、詳細の保持を担当します。`system-controls.js` は記録・ヘルプと新規開始確認の戻り順序を管理します。戦闘の未確定行動と対象はView状態です。選択中にCoreへ送らず、確定時に既存のbattle intentを送ります。キャンセルで逃走・有料帰還・必須判断を代行しないでください。

初期ビューは `SceneView`、従来ビューは `GameView` です。「記録 → 画面配置」で切り替え、settings.viewLayoutへ保存します。`src/view/view-layout.js` が差し替え、同じViewModel・dispatch・セーブを使います。`SceneView` は会話・戦闘・道具・隊・掲示板・地図の既存描画を再利用します。入力の振り分けは `src/view/keyboard.js`、長文分割は `message-pages.js`、背景内配置は `scene-style.css`。描画用ウィンドウの開閉・ページ送りをCoreの進行へ混ぜないでください。[仕様と検証範囲](IN_SCENE_VIEW.md)を参照してください。
...
1. 新しい共通キー操作の実ブラウザ確認が残っています。PR #27の公開画面では「記録 → 画面配置」と両ビューへの切り替えを確認済みです。ローカルプレビューは `ERR_BLOCKED_BY_CLIENT` で拒否されました。今回の23件のキー試験と既存10件の画面試験はNodeのDOMアダプターを使い、実際のCSS・ネイティブフォーカスの検証には代えません。公開後、選択肢の先頭、敵・味方対象、装備と転職、記録枠と設定、新規開始確認から戻る経路を確認してください。320〜1440px・低い横画面・本文24pxで候補一覧とフォーカス枠の収まりも確認してください。
