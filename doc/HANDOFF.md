...
確認日: 2026-09-23。作品版1.16.0。起点はPR #35まで反映済みのmaster `56a87cc`。作業ブランチは `feat/cell-layers`。最優先だったセルレイヤー整理を実装し、通常33マップを移行しました。
...
1. フィールドイベントに近い自動処理は、固有システムの専用フックへ増やし続けず、表現できるものから共通のフィールドイベントへ寄せます。対象には、目的地到着、歩行後のダンジョン危険判定、環境による戦闘開始などを含めます。特に現在 `fire_network.danger` が直接開始している「くらがり」戦闘は、照度などのセル・環境パラメータを条件にし、成立時の処理を戦闘開始、対象を敵または遭遇定義として指定できる条件付きフィールドイベントで表現する案を優先して検証してください。固有システムには照度や保護状態などの値を提供させ、発火・一回性・再発条件・戦闘開始はフィールドイベント側へ集約します。ただし毎フレーム全イベントを走査する構造にはせず、セル進入、移動完了、照度変化、オブジェクト状態変化など、条件に関係する状態が変わった時だけ候補を評価してください。統合によって処理量または判定の複雑さが明確に増えるものは専用処理を維持し、その理由をイベント一覧へ記録します。
2. 調査操作を「便利調べる」と「任意調べる」に分けます。「便利調べる」は現在の足元・正面を調べる操作の主経路とし、状態が変わらず新しい情報も得られないなど、調べる必要のない対象には反応させません。反応すべき対象が一つなら直接実行し、複数ある場合だけ「何を調べる？」の選択肢を出します。「任意調べる」はプレイヤーがセル、正面、オブジェクト、エッジなどの候補から対象を明示的に選べる操作とし、便利調べるが省いた再確認対象にもアクセスできるようにします。候補収集ではセル上の対象と、プレイヤーが向いている正面エッジ上の対象を同列に扱います。たいまつなどはセル上だけでなくエッジにも設置可能とし、プレイヤーがそのエッジへ向いている場合は便利調べるの候補に含めます。同時に現在セルへ篝火があり、油を差す、火を採るなどの有効な操作がある場合は、エッジ上のたいまつとセル上の篝火の両方を選択肢へ出してください。何を「調べる必要がある」と判定するか、便利調べると任意調べるのキー割当、足元セル・正面セル・正面エッジの優先順位、同じ対象に複数操作がある場合の表示、再確認対象の範囲など、ユーザー側で詰める必要がある要件が判明した場合は、推測で実装せず着手前に質問してください。
3. JSONで管理している設定には、それぞれ対応する編集用HTMLを作り、JSONとHTMLを `config/` に整理します。各HTMLは対応JSONをロードして表または詳細フォームに表示し、ブラウザ上で編集し、検証後の全JSONをコピー可能なテキストとして出力できるようにします。出力はキー順・配列順・インデント・改行を安定させ、編集対象外のフィールドを失わないようにしてください。github.devを開くこと、貼り付けること、ブランチ作成、commit、push、PR作成はユーザー側で行うため、この編集HTMLにはGitHub認証、GitHub APIによる書込み、github.devの自動操作を実装しません。生成物と原稿JSONが分かれている場合は原稿を編集対象にし、生成物を直接編集する画面は作らないでください。
4. 前回はローカルChromiumで固定枠・実寸ページ分割・18選択肢・共通キー・三人の配置と従来カードを確認済みです。1280×856、900×700、390×844、320×568、844×390と本文24pxを含みます。再現用はtools/browser-scene-smoke.mjsです。公開反映後の通し操作、全管理画面とキー変更の入力待ち・取消・適用・初期値復帰・再読み込み後の保持、音声は引き続き確認してください。全編の場面に必要な透過spriteと配置はシナリオごとに追加し、関係性を無視した一律配置へ置き換えないでください。
5. q004から物理的な場所・人物と物品の所在・出発・到着を個別に設計します。費用・戦闘・救助・証言は到着前に成立させません。
6. 全200本の文芸監査と、追加100本を含む長期探索の資源・距離・消耗調整を続けます。全世界の時計・NPCスケジュールは未実装です。
