...
確認日: 2026-09-19。作品版1.14.0。起点はPR #28反映済みのmaster `570eae6`。作業ブランチは `feat/ui-philosophy-key-config`。UI文書を `doc/ui/` に集約し、根本方針と現状の整理、キー設定を追加しました。Core・配布JSON・セーブ形式は変更していません。
...
[KEYBOARD_CONTROLS.md](ui/KEYBOARD_CONTROLS.md)が操作仕様です。`keyboard.js` はEnter・矢印・Escapeを共通処理し、`focus.js` が有効なボタンへの移動、選択欄の候補一覧、詳細の保持を担当します。`system-controls.js` は記録・ヘルプと新規開始確認の戻り順序を管理します。戦闘の未確定行動と対象はView状態です。選択中にCoreへ送らず、確定時に既存のbattle intentを送ります。キャンセルで逃走・有料帰還・必須判断を代行しないでください。
...
1. PR #28の共通キー操作と、今回のキー設定の実ブラウザ確認が残っています。PR #27の公開画面では「記録 → 画面配置」と両ビューへの切り替えを確認済みです。ローカルプレビューは `ERR_BLOCKED_BY_CLIENT` で拒否されました。PR #28の23件のキー試験と既存10件の画面試験はNodeのDOMアダプターを使い、実際のCSS・ネイティブフォーカスの検証には代えません。公開後、選択肢の先頭、敵・味方対象、装備と転職、記録枠と設定、新規開始確認から戻る経路に加え、キー変更の入力待ち・取消・適用・初期値復帰・再読み込み後の保持を確認してください。320〜1440px・低い横画面・本文24pxで候補一覧とフォーカス枠の収まりも確認してください。
