2026-09-14：13ダンジョンの壁面・装置素材と、関連依頼に付随する現地調査を追加しました。仕掛けの実状態から観察内容を判定し、依頼一覧・手帳へ記録できます。画像と場面は authoring/dungeon-scenes.json から割り当て・生成します。内容版1.7.0への追加で、旧スクリプトとセーブ形式を維持します。 [素材・現地調査の仕様](DUNGEON_ART_AND_SCENARIOS.md)。

2026-09-14 / v1.7.0：全13件の[ダンジョン一覧](DUNGEON_CATALOG.md)を先に作成し、残る10件の固有設定を実装しました。13ダンジョン・25マップ・200クエスト、全412テスト成功。植物、鏡ワープ、技能貸出、市場取引、空気、配電、手動／自動地形変化、逆行デバフ、境界の術制限をJSONで編集できます。[実装仕様・編集手順](DUNGEON_SYSTEMS_1_7.md)を参照してください。この記録は実装ブランチを対象とし、マージ状況はGitHubのPRで確認してください。

2026-09-14 / v1.6.0：灯守の地下水道に水位周期・水門・バルブ、塩哭きの廃坑に戦闘ごとの装備腐食・スキルとアイテムによる破壊壁を実装しました。全387テストが成功しています。設定JSON、操作、保存・移行、実装上の判断は[実装仕様](WATERWAYS_SALT_MINE.md)を参照してください。以下の版別記録は当時のものです。
...
q011〜q020の既存 `.flow.*` とそれ以前のスクリプトは変更せず、更新前に開始済みの経路は旧本文・旧報酬で終えます。再訪時は既存の場面記録と版印から経路を選び、完了処理は `model.progressionUpgrade.current` によって `outcomes` と `legacyOutcomes` を切り替えます。未開始の依頼は新しい進行へ入ります。JSONだけを修正せず、上記原稿を修正して全体を再生成してください。
