...
確認日: 2026-09-19。作品版1.14.0。起点はPR #26反映済みのmaster `5f029b8`。今回の作業ブランチは `feat/in-scene-view`。ゲーム処理と配布JSONを変えず、背景内ビューと従来ビューの切り替えを追加しました。
...
1. `view-preview.html` の画面配置切り替えと本番の「記録 → 画面配置」を実ブラウザで確認します。今回は接続が `ERR_BLOCKED_BY_CLIENT` で拒否され、実ブラウザ検証は未完了です。NodeのDOMアダプターによる回帰試験は実画面のレイアウト・描画・ネイティブフォーカスを保証しません。320〜1440px、低い横画面、文字サイズ24pxでウィンドウの収まり、全ボタンへの到達、人物と敵の見切れ、狙う敵のクリック、ページ送り後のスクロール、システムダイアログの収まりを確認してください。
