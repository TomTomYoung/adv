...
確認日: 2026-09-23。作品版1.18.0。起点はPR #37まで反映済みのmaster `82ba8d3`。作業ブランチは `feat/inspection-modes`。便利調べる・任意調べる、消費時の選択、エッジ上のたいまつを実装しました。
...
通行値 `.` / `#` と、壁・床・遮光・素材・局所照度・通水属性・セルイベント参照を分離しました。[CELL_LAYERS.md](CELL_LAYERS.md) が実装仕様です。セル種は頻出構成のプリセット、例外地点は項目上書きです。`authoring/cell-layers.json` がセルの正本で、既存マップ原稿のtilesは最終生成でこの原稿の通行投影へ置き換わります。配置物と接続口は元の原稿で管理し、セル位置の変更時には双方の整合を検証してください。
...
`authoring/dungeons/kagaribi.json` のfieldEventsへ通常のくらがり襲撃を移しました。`fire_network.danger` は削除し、火の部品は保護状態・遭遇倍率・強度・燃料などを返します。明るさと魔除けは別です。通常松明は照度8でも魔除けがなければ襲われる仕様を維持しました。共通条件では `field.cell.parameters`、`field.illumination`、`field.environment.<system ID>` を使えます。[イベント仕様](scenarios/EVENT_SYSTEM.md)が記法と保存契約です。
...
1. JSONで管理している設定には、それぞれ対応する編集用HTMLを作り、JSONとHTMLを `config/` に整理します。各HTMLは対応JSONをロードして表または詳細フォームに表示し、ブラウザ上で編集し、検証後の全JSONをコピー可能なテキストとして出力できるようにします。出力はキー順・配列順・インデント・改行を安定させ、編集対象外のフィールドを失わないようにしてください。github.devを開くこと、貼り付けること、ブランチ作成、commit、push、PR作成はユーザー側で行うため、この編集HTMLにはGitHub認証、GitHub APIによる書込み、github.devの自動操作を実装しません。生成物と原稿JSONが分かれている場合は原稿を編集対象にし、生成物を直接編集する画面は作らないでください。
2. 前回はローカルChromiumで固定枠・実寸ページ分割・18選択肢・共通キー・三人の配置と従来カードを確認済みです。1280×856、900×700、390×844、320×568、844×390と本文24pxを含みます。再現用はtools/browser-scene-smoke.mjsです。公開反映後の通し操作、全管理画面とキー変更の入力待ち・取消・適用・初期値復帰・再読み込み後の保持、音声は引き続き確認してください。全編の場面に必要な透過spriteと配置はシナリオごとに追加し、関係性を無視した一律配置へ置き換えないでください。
3. q004から物理的な場所・人物と物品の所在・出発・到着を個別に設計します。費用・戦闘・救助・証言は到着前に成立させません。
4. 全200本の文芸監査と、追加100本を含む長期探索の資源・距離・消耗調整を続けます。全世界の時計・NPCスケジュールは未実装です。
