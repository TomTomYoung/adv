...
test('broken script references and unknown opcodes fail validation',()=>{
  const bad=structuredClone(data);bad.scripts.prologue.commands.push({op:'call',script:'missing'});bad.quests.q001.scripts['q001.v11.entry'].commands.push({op:'eval'});bad.scripts['q001.v11.entry'].commands.push({op:'eval'});const errors=validateContent(bad);assert.ok(errors.some(e=>e.includes('missing')));assert.ok(errors.some(e=>e.includes('eval')));
});
