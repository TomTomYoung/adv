...
test('100 unique authored quests, 300 endings, 20 connected maps and complete references',()=>{
  assert.deepEqual(validateContent(data),[]);assert.equal(Object.keys(data.quests).length,100);assert.equal(new Set(Object.values(data.quests).map(q=>q.title)).size,100);assert.equal(new Set(Object.values(data.quests).flatMap(q=>Object.values(q.outcomes).map(o=>o.text))).size,300);assert.equal(Object.keys(data.maps).length,20);
});
...
test('information stays hidden until observed, choosing disabled options has no effect',()=>{
  const g=newGame();g.dispatch({type:'accept',id:'q001'});assert.equal(JSON.stringify(projectGame(g)).includes(data.quests.q001.model.world.truth),false);exploreSpot(g,data.quests.q001.locations[2],{heal:true});const before=g.save();assert.equal(g.dispatch({type:'choose',id:'informed'}),false);assert.equal(g.save(),before);g.dispatch({type:'choose',id:'leave'});drain(g);
});
test('save/restore during text, choice and battle preserves the continuation and PRNG',()=>{
  const g=new GameEngine(data,8);const copy=new GameEngine(data,99);copy.load(g.save());assert.equal(copy.save(),g.save());drain(g);g.dispatch({type:'accept',id:'q001'});exploreSpot(g,data.quests.q001.locations[2],{heal:true});copy.load(g.save());assert.equal(copy.save(),g.save());g.dispatch({type:'choose',id:'contract'});assert.ok(g.state.battle);copy.load(g.save());fight(g);fight(copy);assert.equal(copy.save(),g.save());assert.equal(g.state.quests.q001.stage,'completed');const reward=g.state.gold;g.dispatch({type:'interact'});drain(g);assert.equal(g.state.gold,reward);
});
test('escape does not complete the quest; defeat rescues the party and leaves a retry',()=>{
  const g=newGame(1);g.dispatch({type:'accept',id:'q001'});exploreSpot(g,data.quests.q001.locations[2],{heal:true});g.dispatch({type:'choose',id:'contract'});const gold=g.state.gold;let tries=0;while(g.state.battle&&tries++<10){g.healAll();g.dispatch({type:'battle',action:'escape'});}drain(g);assert.equal(g.state.quests.q001.stage,'active');assert.equal(g.state.gold,gold);assert.equal(g.state.vm.length,0);
  g.dispatch({type:'interact'});drain(g);g.dispatch({type:'choose',id:'contract'});for(const id of g.state.members)g.state.actors[id].hp=1;let turns=0;while(g.state.battle&&turns++<10)g.dispatch({type:'battle',action:'skill',skill:'guard'});assert.equal(g.state.mode,'town');assert.equal(g.state.quests.q001.stage,'active');assert.ok(g.state.members.every(id=>g.state.actors[id].hp>0));assert.equal(g.state.vm.length,0);
});
