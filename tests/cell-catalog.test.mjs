...
test('the authored 30-type catalog is deployed with named presets, a real poison script and compatible existing device placements',async()=>{
 const source=JSON.parse(await read('config/cell-layers.json'));assert.equal(Object.keys(data.cellTypes).length,30);assert.deepEqual(source.presets,data.cellTypes);for(const p of Object.values(data.cellTypes)){assert.ok(p.name&&p.description);}assert.deepEqual(cellBindingErrors(source,data.dungeons),[]);assert.deepEqual(validateContent(data),[]);assert.ok(data.scripts[data.cellEvents.poison_step.script]);
});
