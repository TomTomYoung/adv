...
export const data=await loadContent(async file=>{const archived=path.resolve(import.meta.dirname,'../authoring/legacy/2026-09-18-map-layout',file);const text=await fs.readFile(archived,'utf8').catch(e=>{if(e.code!=='ENOENT')throw e;return fs.readFile(path.resolve(import.meta.dirname,'..',file),'utf8');});return JSON.parse(text);});
