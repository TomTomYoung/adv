...
// Explicitly exercise the retired voxel/tidal layout; gameplay tests use helpers.mjs.
export const data=await loadContent(async file=>{const archived=path.resolve(import.meta.dirname,'../authoring/legacy/2026-09-18-map-layout',file);const text=await fs.readFile(archived,'utf8').catch(e=>{if(e.code!=='ENOENT')throw e;return fs.readFile(path.resolve(import.meta.dirname,'..',file),'utf8');});const value=JSON.parse(text);if(file==='data/locations.json'){const roots=new Set(Object.values(value).filter(l=>l.dungeonEntrance).map(l=>l.id));for(const [id,l] of Object.entries(value))if(roots.has(id)||roots.has(l.parent))delete value[id];}if(file==='data/dungeons.json')for(const d of Object.values(value))for(const spec of Object.values(d.systems??{}))if(spec.use==='fire_network'&&spec.threat?.encounter){spec.threat={encounterRate:0.22,encounterPool:[{encounter:spec.threat.encounter,weight:1}]};}return value;});
