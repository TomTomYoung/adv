...
import {routeTo} from './structure-routes.mjs';
...
test('full 100-quest campaign is completable from normal starting stats and earned resources',()=>{
  ...
  for(const q of Object.values(data.quests).filter(q=>q.number<=100)){
    ...
    for(const id of routeTo(q.id,'informed')){assert.ok(g.dispatch({type:'choose',id}),q.id+'/'+id);drain(g);if(g.state.battle)fight(g);drain(g);}
