...
test('shallow water has a surface and remains traversable instead of disappearing from the picture',()=>{
 const g=begin();wait(g,12);const d=projectGame(g).dungeon,c=d.cells[2][3];assert.equal(c.waterDepth,1);assert.equal(c.water,true);assert.equal(c.blocked,false);assert.equal(d.geometry[2][3],'.');assert.match(d.surfaceNotice.text,/足元まで・通行可/);
...
test('full water blocks movement but not the sightline, and the displayed wait action restores the route',()=>{
 const g=begin();wait(g,18);const d=projectGame(g).dungeon,c=d.cells[2][3];assert.equal(c.waterDepth,3);assert.equal(c.blocked,true);assert.equal(d.geometry[2][3],'.');
 ...
 for(let i=0;i<8;i++)assert.ok(g.dispatch(projectGame(g).dungeon.surfaceNotice.action.intent));assert.ok(projectGame(g).dungeon.cells[2][3].waterDepth<3);assert.ok(g.dispatch({type:'move',direction:'forward'}));
});
test('closing the matching channel valve removes water visually and opens the same cells in the engine',()=>{
 const g=begin();g.teleport('region_1_f1',2,3,'west');let d=projectGame(g).dungeon;assert.equal(d.cells[3][4].waterDepth,3);assert.equal(g.walkable(g.map(),4,3),false);
...
test('water markers stay on the surface and climbing markers do not acquire water-valve artwork',()=>{
 const g=begin();wait(g,12);assert.ok(projectGame(g).dungeon.objects.filter(o=>o.kind==='water').every(o=>o.art===null));
...
test('permanent channels identify their valve and never offer waiting as a way to drain them',()=>{
 const g=begin();g.teleport('region_1_f1',3,3,'east');const notice=projectGame(g).dungeon.surfaceNotice;assert.match(notice.text,/上層水路のバルブ/);assert.match(notice.text,/待つだけでは水は引きません/);assert.equal(notice.action,null);
});
