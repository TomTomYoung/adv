...
for(const q of quests){catalog.push(`## ${q.id} ${q.title}`,'',`依頼人: ${q.client}。地域: ${q.region}。${q.unlockHint}`,'',q.brief,'',`モデル: ${q.model.standard.version}。実装: [JSON](../data/quests/${q.id}.json)。場面 ${q.model.graph?.length??0}、結末 ${Object.keys(q.outcomes).length}。`,'',`固定された過去: ${q.model.world.truth}`,'');for(const [k,o] of Object.entries(q.outcomes))catalog.push(`${k} — ${o.label}（${o.gold}G / ${o.xp}EXP）: ${o.text}`,'');catalog.push(`進行: ${q.model.progression??'各場面の選択に従う。'}`,'');for(const n of q.model.graph??[])catalog.push(`- ${n.id}: ${n.options.map(o=>`${o.text} → ${o.to}${o.combat?'［戦闘］':''}`).join(' / ')}`);catalog.push('');}
