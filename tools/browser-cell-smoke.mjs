...
const presets=JSON.parse(await fs.readFile(new URL('../config/cell-layers.json',import.meta.url),'utf8')).presets;
...
try{
 ...
 await page.goto(base+'config/map.html?dungeon=kagaribi');await page.locator('.palette-choice').first().waitFor();assert.equal(await page.locator('.palette-choice').count(),30);
 await page.getByRole('button',{name:'配置する',exact:true}).click();await page.getByRole('button',{name:presets['poison_swamp'].name,exact:true}).click();await page.locator('[data-cell="2,1"]').click();assert.equal(await page.locator('[data-cell="2,1"]').evaluate(e=>e.parentElement.dataset.surface),'poison');
 ...
 await page.getByRole('button',{name:'設定を調べる',exact:true}).click();await page.locator('[data-cell="2,1"]').click();await page.getByRole('button',{name:'セル進入イベントを編集：毒沼への進入',exact:true}).click();await page.getByRole('button',{name:'会話・選択肢・消費・報酬を編集',exact:true}).click();await page.locator('.record-detail').getByText('毒の沼地を踏んだ。',{exact:false}).first().waitFor({state:'attached'});
 await page.getByRole('button',{name:'セル',exact:true}).click();await page.getByRole('button',{name:'配置する',exact:true}).click();await page.getByRole('button',{name:presets['salt_wall'].name,exact:true}).click();await page.locator('[data-cell="3,1"]').click();await page.locator('.status.error').waitFor();assert.match(await page.locator('.status.error').innerText(),/対象地点/);
 await page.getByRole('button',{name:presets['ice_floor'].name,exact:true}).click();await page.locator('[data-cell="3,1"]').click();await page.getByRole('button',{name:presets['wood_floor'].name,exact:true}).click();await page.locator('[data-cell="4,1"]').click();await page.getByRole('button',{name:presets['earth_floor'].name,exact:true}).click();await page.locator('[data-cell="5,1"]').click();
 ...
 assert.deepEqual(errors,[]);console.log('CELL BROWSER: 30 named brushes, material previews, poison placement / script / JSON export, invalid system binding rejection, responsive widths and actual game canvas rendering passed');
