import {docPath,relocateDoc,currentMarkdown} from './doc-layout.mjs';
...
await fs.writeFile(path.join(folder,docPath('EVENT_CATALOG.md')),relocateDoc(eventCatalog(data),'EVENT_CATALOG.md'));
await fs.writeFile(path.join(folder,docPath('CHARACTERS.md')),relocateDoc(characterCatalog(data),'CHARACTERS.md'));
await fs.writeFile(path.join(folder,docPath('LOCATION_CATALOG.md')),relocateDoc(locationCatalog(data),'LOCATION_CATALOG.md'));
...
const snapshot={updated:date,contentVersion:version,contentSha256:digest.digest('hex'),method:'game manifestの配布JSONを読み込み、ID数と現在のoutcomes/model.graphを集計。旧スクリプトを含む。story.scenesは互換場面名も含む。',counts:Object.fromEntries(['locations','dungeons','maps','quests','scripts','actors','characters','jobs','skills','buffs','fieldAbilities','items','enemies','encounters','sounds','effects','statuses'].map(k=>[k,count(data[k])])),questOutcomes:quests.reduce((n,q)=>n+count(q.outcomes),0),questGraphScenes:quests.reduce((n,q)=>n+(q.model.graph?.length??0),0),typedStoryQuests:quests.filter(q=>q.story).length,typedStoryScenesIncludingAliases:quests.reduce((n,q)=>n+count(q.story?.scenes),0),questEvents:questEvents(data).length,questEventPlacements:questEvents(data).filter(e=>!['action','auto'].includes(e.trigger)).reduce((n,e)=>n+e.points.length,0),questObservations:questEvents(data).filter(e=>e.note).length,commands:[...COMMANDS],operators:[...EXPRESSION_OPS],migrationVersions:Object.keys(data.game.migrations),assets:{images:count(data.assets.images),audio:count(data.assets.audio)},files};
await fs.writeFile(path.join(folder,docPath('DATA_SNAPSHOT.json')),JSON.stringify(snapshot,null,2)+'\n');
async function section(name,key,text){const file=path.join(folder,docPath(name)),begin=`<!-- generated:${key} -->`,end=`<!-- /generated:${key} -->`,block=`${begin}\n\n${relocateDoc(text.trim(),name)}\n\n${end}`;let s=await fs.readFile(file,'utf8');if(s.includes(begin)){const a=s.indexOf(begin),b=s.indexOf(end,a);if(b<0)throw Error(`Unclosed documentation section: ${name}`);s=s.slice(0,a)+block+s.slice(b+end.length);}else s=s.trimEnd()+'\n\n'+block+'\n';await fs.writeFile(file,s);}
...
for(const name of ['QUEST_CATALOG.md']){const f=path.join(folder,docPath(name));let s=await fs.readFile(f,'utf8');s=s.replace(/作品版 \d+\.\d+\.\d+。/g,`作品版 ${version}。`).replace(/更新日: \d{4}-\d{2}-\d{2}。/g,`更新日: ${date}。`);await fs.writeFile(f,s);}
...
for(const file of await currentMarkdown(folder)){const p=path.join(folder,file);await fs.writeFile(p,plainMarkdown(await fs.readFile(p,'utf8')));}
