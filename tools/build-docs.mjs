...
const snapshot={updated:date,contentVersion:version,contentSha256:digest.digest('hex'),method:'game manifestの配布JSONを読み込み、ID数と現在のoutcomes/model.graphを集計。旧スクリプトを含む。story.scenesは互換場面名も含む。',counts:Object.fromEntries(['dungeons','maps','quests','scripts','actors','characters','jobs','skills','buffs','fieldAbilities','items','enemies','encounters','sounds','effects','statuses'].map(k=>[k,count(data[k])])),questOutcomes:quests.reduce((n,q)=>n+count(q.outcomes),0),questGraphScenes:quests.reduce((n,q)=>n+(q.model.graph?.length??0),0),typedStoryQuests:quests.filter(q=>q.story).length,typedStoryScenesIncludingAliases:quests.reduce((n,q)=>n+count(q.story?.scenes),0),fieldScenes:Object.values(data.dungeons).reduce((n,d)=>n+(d.fieldScenes?.length??0),0),commands:[...COMMANDS],operators:[...EXPRESSION_OPS],migrationVersions:Object.keys(data.game.migrations),assets:{images:count(data.assets.images),audio:count(data.assets.audio)},files};
