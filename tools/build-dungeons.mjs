...
export async function buildDungeons(){
  ...
  const definitions={};
  for(const file of (await fs.readdir(path.join(root,'config/dungeons'))).filter(f=>f.endsWith('.json')).sort()){
    const d=await read(`config/dungeons/${file}`);if(Object.hasOwn(definitions,d.id))throw Error(`Dungeon ID duplicated: ${d.id}`);
  ...
  const content=await read('config/kagaribi-content.json'),terrain=await read('config/terrain-content.json'),extra=await read('config/dungeon-content.json'),voxel=await read('config/voxel-content.json'),game=await read('data/game.json');
  ...
  const connected=await read('config/connected-maps.json');
