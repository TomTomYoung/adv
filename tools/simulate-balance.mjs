...
export function chooseBattleAction(g){
  const s=g.state,b=s.battle,id=activeActor(g),actor=s.actors[id],known=g.data.actors[id].skills;
  ...
  const can=skill=>known.includes(skill)&&actor.mp>=g.data.skills[skill].mp;
  ...
  const empty=allies.find(other=>other!==id&&s.actors[other].mp<4&&g.data.actors[other].skills.some(skill=>['fire','ice','lightning','group_heal'].includes(skill)));
...
export function simulateBalance(data,{seeds=20}={}){
  ...
  for(const [team,members] of Object.entries(teams))for(const id of ids){
    ...
    for(let seed=1;seed<=seeds;seed++){
      ...
      g.state.members=[...members];g.state.level=region;g.state.xp=data.system.xpBase*(region-1)*region;g.healAll();g.dispatch({type:'travel',region});g.startBattle(id,{win:[],lose:[],escape:[]});
