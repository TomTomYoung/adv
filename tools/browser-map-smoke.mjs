...
try{
 ...
 await page.locator('.record-list button').filter({hasText:'西の壁松明'}).click();await page.getByLabel('表示名',{exact:true}).fill('統合画面の壁灯');assert.equal(await page.getByLabel('編集するマップ',{exact:true}).inputValue(),'kagaribi_f1');
