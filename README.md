...
最初は掲示板で「帰らない灯番」を受注し、その場で「迷宮の入口へ向かう（篝火の迷宮）」を押します。広場へ戻る必要はありません。W/Sで前後移動、A/Dで方向転換します。新人・老人のイベントは指定セルを踏むと自動開始し、Eで壁灯などの足元・正面を調べます。ボタンでも全操作ができます。メインクエストと手帳に次の目的地が出ます。

初期表示は背景内ビューです。「記録 → 画面配置」で従来表示へ切り替えられます。道具・隊・手帳は背景内に開き、Escapeまたは「閉じる」で戻ります。長文の「次のページ」は表示だけを送り、最後の「続きを読む」で物語を進めます。[表示仕様](doc/IN_SCENE_VIEW.md)。
