...
2026-09-14：13ダンジョンの壁面・装置素材と、関連依頼に付随する現地調査を追加しました。仕掛けの実状態から観察内容を判定し、依頼一覧・手帳へ記録できます。画像と場面は authoring/dungeon-scenes.json から割り当て・生成します。内容版1.7.0への追加で、旧スクリプトとセーブ形式を維持します。 [素材・現地調査の仕様](doc/DUNGEON_ART_AND_SCENARIOS.md)。
