...
追加の魔物・仲間の原稿は `authoring/entities.json`。`node tools/build-entities.mjs` で拡張データを再生成します。build-contentも最後に同じ拡張を適用します。`node tools/simulate-balance.mjs` で1,200戦の計測結果をdocへ書き出します。1.0.0の記録は1.1.0へ自動移行し、既存の進行を保持します。
