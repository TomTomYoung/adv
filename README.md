...
最初は掲示板で「帰らない灯番」を受注し、その場で「迷宮の入口へ向かう（篝火の迷宮）」を押します。広場へ戻る必要はありません。町では矢印でコマンドを選び、Enterで決定します。ダンジョン探索中は↑/↓で前進・後退、←/→で方向転換し、Enterで壁灯などの足元・正面を調べます。W/S/A/DとEも補助キーとして使えます。新人・老人のイベントは指定セルを踏むと自動開始します。会話・選択肢・戦闘もEnterと矢印で進められます。メインクエストと手帳に次の目的地が出ます。
...
町のキャラクターカードからプロフィールの「転職」を開き、30職から転職先を選んで確定します。能力・成長・習得段階・袋へ戻る装備を確認できます。転職は無料ですが回復はせず、過去の成長は保持します。測量・調合はプロフィールの「スキル」、祈祷・点火などは「魔法」ページから使用します。「隊の状態」は左上にパーティーのカードと待機、左下に酒場のカードと加入、各右側に選択した人物のプロフィールを表示します。[プロフィールと編成](doc/ui/CHARACTER_PROFILES.md)。
...
内容版1.18.0は旧セーブを移行しません。自動保存・スロット・ファイルの読込でエラーが出たら、最初から新しい旅を始めます。正常な同版の記録は会話・戦闘を含め再開できます。
