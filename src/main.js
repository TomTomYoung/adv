...
function modal(title){dialog.replaceChildren();const h=make('h2',title);h.id='dialog-title';dialog.append(h);syncSystemPlacement();if(!dialog.open)dialog.showModal();}
...
function help(){modal('遊び方');for(const paragraph of [
 ...
 '2. W/Sまたは前後ボタンで移動、A/Dで方向を変えます。Eか「調べる」で足元と正面を調べます。メインクエスト欄の座標は横・縦の順です。',
...
const viewUi={menu,help,status,layoutChanged:syncSystemPlacement,cancelFeedback:()=>sound.stopEffects(),effectsMode:()=>settings.effects,soundEnabled:()=>settings.sound,soundLabel:()=>sound.label(),sound:toggleSound};
try{
  ...
  document.addEventListener('keydown',event=>handleGameKey(event,{view,model:lastModel,dispatch,menu,modalOpen:dialog.open}));
