...
let engine,view,data,lastModel,statusTimer,settings={volume:.25,sound:false,theme:THEME_DEFAULT},audio=null,currentMusic=null,lastSeRevision=0;
...
function syncAudio(){
  if(!settings.sound){audio?.pause();return;}
  if(lastModel.se?.url&&lastModel.se.revision!==lastSeRevision){lastSeRevision=lastModel.se.revision;const effect=new Audio(lastModel.se.url);effect.volume=settings.volume;effect.play().catch(()=>{});}
  if(lastModel.music!==currentMusic){audio?.pause();currentMusic=lastModel.music;audio=currentMusic?new Audio(currentMusic):null;if(audio){audio.loop=true;audio.addEventListener('error',()=>status('音声を読み込めません。音を切ったままでも探索を続けられます。'));}}
  if(audio){audio.volume=settings.volume;audio.play().catch(()=>status('音の再生には画面の「音」を押してください。'));}
}
...
function help(){modal('遊び方');for(const paragraph of [
 ...
 '5. 旅支度で補給・装備、隊の編成で宿・無料施療所・五人目の仲間を利用。入口から無料帰還、帰還印は所持金8%、全滅時は15%を失います。依頼と手掛かりは残ります。',
...
try{
  const savedSettings=storageRead('settings');if(savedSettings){try{const parsed=JSON.parse(savedSettings);settings.volume=Number.isFinite(parsed.volume)?Math.max(0,Math.min(1,parsed.volume)):.25;settings.theme=applyTheme(parsed.theme??THEME_DEFAULT);}catch{applyTheme(THEME_DEFAULT);}}else applyTheme(THEME_DEFAULT);
  ...
  view=new GameView(root,dispatch,{menu,help,retreat,status,soundEnabled:()=>settings.sound,sound:()=>{settings.sound=!settings.sound;render();}});render();
