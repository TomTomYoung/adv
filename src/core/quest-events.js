...
export function validateQuestEvents(data,expression){
  ...
  for(const q of Object.values(data.quests)){
    ...
    for(const event of q.events){
      ...
      if(typeof event.title!=='string'||!event.title||!['enter','interact','action'].includes(event.trigger))fail(id,'イベントの名前・起動方法が不正です');
      ...
      if(!Array.isArray(event.points)||!event.points.length)fail(id,'配置・調査地点が必要です');
