...
export function addGear(state,item,count){
 ...
 else for(let i=0;i<-count;i++){const shared=bag.findIndex(id=>!g.items[id].holder),personal=state.members.map(actor=>bag.findIndex(id=>g.items[id].holder===actor)).find(at=>at>=0);const [id]=bag.splice(shared<0?(personal??bag.length-1):shared,1);if(!id)throw Error('装備個体の所持数が一致しません');delete g.items[id];}
}
export function equipGear(state,actor,slot,item,holder){
 const g=state.gear,bag=g.bag[item]??[];if(!bag.length)throw Error('装備個体がありません');
 bag.sort((a,b)=>g.items[a].salt-g.items[b].salt);const index=bag.findIndex(id=>holder===undefined||(g.items[id].holder??'shared')===holder);
 if(index<0)throw Error('選んだ持ち主の装備個体がありません');
 const [next]=bag.splice(index,1),slots=g.equipped[actor]??={};delete g.items[next].holder;
 if(slots[slot]){const old=slots[slot];(g.bag[g.items[old].item]??=[]).push(old);if(state.members.includes(actor))g.items[old].holder=actor;}
 slots[slot]=next;
}
export function unequipGear(state,actor,slot){
 ...
 if(state.members.includes(actor))g.items[id].holder=actor;
}
...
export function gearErrors(data,state){
 ...
 for(const [actor,a] of Object.entries(state.actors))for(const [slot,item] of Object.entries(a.equipment)){const id=g.equipped[actor]?.[slot];check(id,item);if(g.items[id]?.holder!==undefined)errors.push('装備中の品物が持ち物にも存在します');}
 ...
 for(const c of Object.values(g.items))if(c.holder!==undefined&&!state.members.includes(c.holder))errors.push('装備の保持者が不正です');
 return errors;
}
