...
import {exact,idList,pointsValid,cellsValid,patchValid,patchTile,patchCell,sameCell,action,markers,panel} from './environment.js';
...
export const airSupply={createPersistent:()=>({raised:[]}),createRun:s=>({air:s.capacity}),plan,
  ...
  cell:(ctx,map,x,y)=>patchCell(ctx.spec.devices.filter(d=>ctx.persistent.raised.includes(d.id)).flatMap(d=>d.tiles),map,x,y),
