...
export const patchValid=(data,d,patches)=>cellsValid(data,d,patches)&&patches.every(p=>['.','#'].includes(p.tile)&&!data.maps[p.map].objects.some(o=>o.x===p.x&&o.y===p.y)&&!sameCell({...data.maps[p.map].entrance,map:p.map},p));
