...
function plan(ctx,intent){
  if(intent.action==='visit'){const portal=ctx.spec.portals.find(p=>p.id===intent.target);if(!portal||!closeTo(ctx.state,portal.at))return {ok:false,reason:'立坑への入口で操作してください。'};const dest=portal.destination;if(ctx.data.maps[dest.map]?.voxels){const terrain=ctx.persistent.maps[dest.map]??freshVoxelState(ctx.data.maps[dest.map]);const reason=voxelOccupancyReason(ctx.data.maps[dest.map],terrain,dest);if(reason)return {ok:false,reason};}return {ok:true,portal};}
  ...
  if(intent.action==='traverse'||intent.action==='install'){
    ...
    const reason=voxelRouteReason(map,terrain,path);if(reason)return {ok:false,reason};
  ...
  if(intent.action==='pump'&&device.kind==='pump'){
    ...
    if(flow.rejected)return {ok:false,reason:'密閉空間が満水です。先に排水口を開いてください。'};
...
function evacuate(ctx,map,before){
  ...
  if(!voxelOccupancyReason(map,after,start))return;
  ...
  for(let i=0;i<queue.length;i++){
    const p=queue[i];if(!voxelOccupancyReason(map,after,p)&&engine.walkable(map,p.x,p.y,p.z)){Object.assign(state.location,p);engine.reveal();engine.notify('増水・足場の変化により、通行できた経路から安全な足場へ退避しました。');return;}
...
function act(ctx,intent,p){
  ...
  else{ctx.persistent.maps[p.map.id]=p.after;ctx.engine.notify(intent.action==='pump'?`${p.device.name}で${p.device.amount}単位を給水しました。`:intent.action==='dig'?`${p.device.name}を掘削し、空の立方体にしました。`:`${p.face.name}を${p.after.faces[p.face.id]?'開きました':'閉じました'}。`);}
  ctx.run.elapsed++;evacuate(ctx,p.map,before);
}
function project(ctx){
  ...
  for(const d of map.voxels.devices)if(near(ctx,c,d.at)){
    const actions=d.kind==='pump'?[action({action:'pump',target:d.id},`${d.amount}単位を給水`)]:[action({action:'dig',target:d.id,item:d.item},`${ctx.data.items[d.item].name} ×${d.count}`),...ctx.state.members.filter(id=>d.ability&&permission(ctx.data,ctx.state,id,d.ability,'wall.break')).map(actor=>action({action:'dig',target:d.id,actor,ability:d.ability},`${ctx.data.actors[actor].name}の${ctx.data.fieldAbilities[d.ability].name}`))];
    cards.push({name:d.name,text:d.kind==='pump'?'供給した水は、六面の接続と空間の容量に従ってたまるか排水されます。':terrain.removed.includes(voxelKey(d.target))?'掘削済み。':'許可された材料か技能で、密の立方体を空にします。',actions});
  }
  ...
  return {kind:'voxel_space',id:ctx.id,title:'貯水立坑の地形と水',summary:`高さ ${loc.z??0} ／ 足元：${depthName(voxelDepth(terrain,loc))} ／ 貯水 ${Object.values(terrain.water).reduce((n,v)=>n+v,0)}単位 ／ 排水累計 ${terrain.drained}単位`,cards,actions:[],markers};
}
