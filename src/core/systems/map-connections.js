import {object,identifier,validPoint,faces,available} from './common.js';
  ...
  validate(data,d,s){
    ...
    for(const l of s.links){
      ...
      for(const p of [l.a,l.b]){
        ...
        if(l.kind!=='stairs'){
          const delta=faces[p.side];if(!delta||data.maps[p.map].tiles[p.y+delta[1]]?.[p.x+delta[0]]!=='#')errors.push('扉は壁に接する通路の端へ置いてください');
