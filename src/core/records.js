...
export const freshRecords=(historyComplete=true)=>({historyComplete,battles:0,wins:0,escapes:0,losses:0,repels:0,kills:{},encounters:{},baselines:{}});
export function snapshotRecords(records){return {battles:records.battles,wins:records.wins,escapes:records.escapes,losses:records.losses,repels:records.repels??0,kills:{...records.kills},encounters:{...records.encounters}};}
...
export function recordResult(engine,result){
  ...
  const key={win:'wins',escape:'escapes',lose:'losses',repel:'repels'}[result];r[key]=(r[key]??0)+1;
...
export function recordsValid(r,data){
  ...
  const counts=v=>v&&['battles','wins','escapes','losses'].every(k=>n(v[k]))&&dict(v.kills,data.enemies)&&dict(v.encounters,data.encounters)&&n(v.repels??0)&&v.wins+v.escapes+v.losses+(v.repels??0)<=v.battles;
  return typeof r?.historyComplete==='boolean'&&counts(r)&&r.baselines&&typeof r.baselines==='object'&&!Array.isArray(r.baselines)&&Object.entries(r.baselines).every(([id,b])=>Object.hasOwn(data.quests,id)&&counts(b)&&['battles','wins','escapes','losses','repels'].every(k=>(b[k]??0)<=(r[k]??0))&&['kills','encounters'].every(k=>Object.entries(b[k]).every(([id,n])=>n<=(r[k][id]??0))));
}
