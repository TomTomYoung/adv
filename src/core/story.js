...
export function storyStateErrors(d,s,state,id,{scene=false}={}){
  ...
  if(scene&&s.scene&&d.scenes[s.scene])try{assertScene(d,s,state,id,s.scene);}catch(e){errors.push(e.message);}
  return errors;
}
...
export function storyActionPlan(data,state,id,actionId){
  const d=definition(data,id),current=state.stories?.[id],a=d.actions[actionId];
