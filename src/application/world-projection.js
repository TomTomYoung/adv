...
import {atWorldPlace,worldPlaceName,townLocation,dungeonInterior} from '../core/world.js';
...
export function projectWorld(engine,dialog){
  ...
  const interior=dungeonInterior(d,s);
  ...
  return {journey,town:{id:location.id,name:location.name,description:location.description,background:d.assets.images[location.background],parent:location.parent?{id:location.parent,name:d.locations[location.parent].name}:null,interior:Boolean(interior),exit:interior&&!location.parent?{label:location.exitLabel??`${location.name}を出る`,intent:{type:'location.exit'}}:null,breadcrumbs,links,cast,stories,shop:Boolean(location.shop),party:Boolean(location.party),quests:Boolean(location.quests),dungeons:(location.dungeons??[]).map(id=>({id,name:d.dungeons[id].name})),services:d.game.services.filter(v=>location.services?.includes(v.id)),busy}};
}
