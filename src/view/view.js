...
export class GameView {
  ...
  render(model){
    ...
    const toolbar=node('div','toolbar');toolbar.append(button('手帳',()=>{this.tab='journal';this.render(model);}),button('記録',()=>this.ui.menu()),button('音：'+(this.ui.soundEnabled()?'入':'切'),()=>this.ui.sound()));header.append(brand,toolbar);this.root.append(header);
    ...
    const allTabs=model.mode==='town'?[['quests','依頼掲示板'],['regions','迷宮へ'],['party','隊の編成'],['bag','旅支度'],['journal','冒険手帳']]:[['explore','探索'],['bag','道具'],['party','隊の状態'],['journal','冒険手帳']];
  ...
  battle(parent,m){
    ...
    const select=node('select');select.setAttribute('aria-label','回復・道具の対象');for(const a of m.party.filter(a=>a.hp>0))select.append(new Option(`${a.name}　HP ${a.hp}/${a.maxHp}`,a.id));select.value=this.selectedAlly;select.addEventListener('change',()=>{this.selectedAlly=select.value;});section.append(select);
    const skills=node('div','battle-actions');for(const s of m.battle.skills){const b=button(`${s.name}${s.mp?' MP'+s.mp:''}`,()=>this.act({type:'battle',action:'skill',skill:s.id,target:s.target==='ally'?this.selectedAlly:s.target==='self'?m.battle.actorId:this.selectedTarget}),'',!s.enabled);b.title=s.description??s.name;skills.append(b);}for(const item of m.battle.items)skills.append(button(`${item.name} ×${item.count}`,()=>this.act({type:'battle',action:'item',item:item.id,target:this.selectedAlly})));skills.append(button('逃走',()=>this.act({type:'battle',action:'escape'}),'',!m.battle.canEscape));section.append(skills);
  ...
  party(parent,m){const section=node('div','panel-content');section.append(heading('COMPANIONS','帰り道をともにする隊'));for(const a of m.party){const row=node('article','actor-detail');row.append(node('h3','',`${a.name} / ${a.class}`),node('p','',a.role),node('p','muted',`HP ${a.hp}/${a.maxHp}　MP ${a.mp}/${a.maxMp}　攻 ${a.stats.str}　防 ${a.stats.vit}　知 ${a.stats.int}`),node('p','',Object.values(a.equipment).join(' / ')||'装備：標準の旅装'));section.append(row);}if(m.mode==='town')for(const service of m.services){const row=node('div','service');row.append(button(service.label,()=>this.act({type:'service',id:service.id})),node('p','muted',service.detail));section.append(row);}parent.append(section);}
  bag(parent,m){const section=node('div','panel-content');section.append(heading('PROVISIONS','持ち物と旅支度'));const target=node('select');target.setAttribute('aria-label','道具・装備の対象');for(const a of m.party)target.append(new Option(`${a.name} (${a.class})`,a.id));section.append(target);
    for(const item of m.inventory){const row=node('article','item-row'),body=node('div');body.append(node('h3','',`${item.name} ×${item.count}`),node('p','muted',item.description));row.append(body);if(item.field||item.slot)row.append(button(item.slot?'装備する':'使う',()=>this.act({type:item.slot?'equip':'item',item:item.id,actor:target.value})));section.append(row);}
  ...
  journal(parent,m){const section=node('div','panel-content');section.append(heading('FIELD NOTES','冒険手帳'));if(m.ending){const ending=node('article','ending');ending.append(node('span','eyebrow','終幕'),node('h2','',m.ending.title),node('p','',m.ending.text));section.append(ending);}
    const active=m.quests.filter(q=>q.stage==='active');if(active.length){section.append(node('h3','','受注中'));for(const q of active)section.append(button(`${q.tracked?'◆ ':''}${q.title} / 手掛かり ${q.evidenceCount}/2`,()=>this.act({type:'track',id:q.id}),'journal-track'));}
  ...
  sidebar(parent,m){
    const party=node('section','side-section');party.append(node('span','eyebrow','冒険者の隊'));for(const a of m.party){const row=node('div',`party-row ${a.hp<=0?'fallen':''}`),avatar=node('div','actor-symbol',a.class[0]);avatar.style.color=a.color;const body=node('div','party-body');body.append(node('div','party-name',`${a.name}　${a.class}${a.statuses.length?' / '+a.statuses.join('・'):''}`),node('div','party-values',`HP ${a.hp}/${a.maxHp}　MP ${a.mp}/${a.maxMp}`),meter(a.hp,a.maxHp,'hp'),meter(a.mp,a.maxMp,'mp'));row.append(avatar,body);party.append(row);}parent.append(party);
    ...
    if(m.tracked){const tracked=node('section','side-section tracked-note');tracked.append(node('span','eyebrow','追跡中の依頼'),node('h3','',m.tracked.title),node('p','muted',m.tracked.brief),node('p','',`手掛かり ${m.tracked.evidenceCount} / 2`));for(const loc of m.tracked.locations){const floor=loc.map.endsWith('f1')?1:2;tracked.append(node('p','coordinates',`地下${floor}層 (${loc.x}, ${loc.y}) / ${loc.role==='decision'?'決着':loc.role==='clue_a'?'痕跡':'記録・証言'}`));}if(m.mode==='town')tracked.append(button('この迷宮へ',()=>this.act({type:'travel',region:m.tracked.region}),'primary'));parent.append(tracked);}
