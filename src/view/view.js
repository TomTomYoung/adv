...
export class GameView {
  ...
  party(parent,m){
    ...
    for(const active of town?[true,false]:[true]){
      ...
      for(const a of roster.filter(a=>!town||(a.active??true)===active)){
        ...
        if(town){const actions=node('div','companion-actions');
  ...
  sidebar(parent,m){
    const party=node('section','side-section');party.append(node('span','eyebrow','冒険者の隊'));for(const a of m.party){const row=node('div',`party-row ${a.hp<=0?'fallen':''}`),avatar=this.portrait(a,'actor-symbol actor-thumb');const body=node('div','party-body');body.append(node('div','party-name',`${a.name}　${a.class}${a.statuses.length?' / '+a.statuses.join('・'):''}`),node('div','party-values',`HP ${a.hp}/${a.maxHp}　MP ${a.mp}/${a.maxMp}`),meter(a.hp,a.maxHp,'hp'),meter(a.mp,a.maxMp,'mp'));row.append(avatar,body);party.append(row);}parent.append(party);
