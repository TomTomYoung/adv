...
export class SystemControls{
  constructor(dialog,fallback=()=>null){
    this.dialog=dialog;
  ...
  open(title,parent=null){
    ...
    if(parent)this.parentSnapshot=captureFocus(d);
    if(!d.open)this.opener=document.activeElement;
    this.parent=parent;this.title=title;d.replaceChildren();
  ...
  handleKey(event){
    ...
    if(event.key==='Escape'){event.preventDefault();this.back();return true;}
    if(isEditing(document.activeElement))return true;
    const scope=this.dialog.querySelector('.button-picker')??this.dialog;
    if(moveFocus(scope,event))return true;
    if(['Enter',' '].includes(event.key)){event.preventDefault();if(!event.repeat){const current=document.activeElement;const target=buttons(scope).includes(current)?current:buttons(scope)[0];target?.click();}return true;}
