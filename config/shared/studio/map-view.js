...
import {edgeKey,edgeSides} from '../../../src/core/edge-layers.js';
...
export function mapPanel(app,{mapId,target=null,paint=false,onInspect,selectedCell=null,z=0,mapOptions,onMapChange,onPlace,compact=false,showMapSelect=true,components=false,showMarkers=true}={}){
 ...
 if(components)root.append(app.componentTools());else{toolbar.append(button(app.showLight?'照度を隠す':'初期照度を表示',()=>{app.showLight=!app.showLight;app.refresh();}));root.append(toolbar);}
 notes.append(el('p',components?(app.task==='passage'?'○：セル通行可、×：セル通行不可。緑線：エッジ通行可、赤線：エッジ通行不可。':app.task==='edges'?'境界の種類を選んでセルの四辺へ配置します。両側から同じ境界を編集できます。':app.task==='size'?'右側で寸法を変更します。セルや配置の座標は左上を基準に維持します。':'種類を選んでセルへ配置します。地点の例外は「設定を調べる」で変更できます。'):target?'配置先のセルをクリックしてください。面のある対象は、セル四辺の細いボタンで向きも指定できます。':paint?'セル種を選んで塗るか、セルを選んで通行・外観・照度を個別に編集します。':'配置を選ぶと、そのイベントや仕掛けの編集を開きます。','muted'));
 ...
 const click=(x,y,edge)=>{if(!app.guard())return;try{
  if(components){if(app.task==='size')return;app.changeComponent(x,y,edge);}
  else if(onPlace){onPlace(x,y,edge);}
 ...
 for(const row of view.cells)for(const cell of row){
  const {x,y}=cell,markers=showMarkers?view.markers.filter(p=>p.x===x&&p.y===y):[],tile=el('div','','map-tile'+(cell.visual.wall?' wall':' floor'));
  if(selectedCell?.x===x&&selectedCell?.y===y&&!selectedCell.side)tile.classList.add('selected');if(target&&markers.some(p=>p.file===target.file&&JSON.stringify(p.path)===JSON.stringify(target.path)))tile.classList.add('placed');
  const title=`${x},${y} ${cell.passage==='#'?'通行不可':'通行可能'}${markers.length?' / '+markers.map(p=>p.name).join('、'):''}`;
  const hit=button(components&&app.task==='passage'?(cell.passage==='#'?'×':'○'):app.showLight?String(cell.light):markers.length?String(markers.length):cell.passage==='#'?'':'·',()=>click(x,y),'cell-hit');hit.setAttribute('aria-label',title);hit.dataset.cell=`${x},${y}`;tile.title=(cell.name??'')+' '+title;tile.dataset.preset=cell.preset??'';if(!cell.visual.wall&&!cell.visual.floor)tile.dataset.void='true';if(components&&app.task==='passage')tile.dataset.passage=cell.passage;tile.dataset.surface=cell.visual.surface??'stone';if(cell.parameters.water_depth)tile.dataset.depth=cell.parameters.water_depth;tile.append(hit);if(!(components&&app.task==='passage'))reliefPreview(tile,cell,[view.cells[y-1]?.[x],view.cells[y]?.[x+1],view.cells[y+1]?.[x],view.cells[y]?.[x-1]]);
  if(app.showLight){tile.style.setProperty('--light',String(cell.light/8));tile.style.filter=`brightness(${.35+.65*cell.light/8})`;}
  ...
  for(const side of edgeSides){const edge=view.edges.edges[`${x},${y}/${side}`];if(edge){const mark=el('span','','boundary-mark '+side);mark.dataset.passage=edge.passage;mark.dataset.surface=edge.visual.surface??'stone';mark.setAttribute('aria-hidden','true');tile.append(mark);}}
  if(target?.allowEdge||components&&['edges','passage'].includes(app.task))for(const side of edgeSides){const hit=button('',()=>click(x,y,side),'edge-hit '+side);hit.setAttribute('aria-label',`${x},${y}の${enumLabel(side)}面に配置`);hit.dataset.edge=`${x},${y}/${side}`;if(components){hit.setAttribute('aria-label',`${x},${y}の${enumLabel(side)}境界`);if(selectedCell?.side&&edgeKey(x,y,side)===edgeKey(selectedCell.x,selectedCell.y,selectedCell.side))hit.classList.add('selected-edge');}tile.append(hit);if(!(components&&app.task==='passage'))reliefPreview(tile,cell,[view.cells[y-1]?.[x],view.cells[y]?.[x+1],view.cells[y+1]?.[x],view.cells[y]?.[x-1]]);}
  grid.append(tile);
 }
 ...
 const points=!showMarkers?[]:selectedCell?view.markers.filter(p=>p.x===selectedCell.x&&p.y===selectedCell.y):view.markers;
